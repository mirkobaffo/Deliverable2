...
public class CsvWriter {
	...
	public static void main(String[] args) throws Exception {
		...
		getReleaseInfo.assignCommitListToRelease(releases.subList(0, size/2), commit);
		Metrics.getMetrics(commit, releases.subList(0, size/2));
		...
		System.out.println("release 1: " + releases.get(0).getCommit().getId() + "release 2: " + releases.get(1).getCommit().getId());
		csvFinal(releases.subList(0,size/2), 0);
		...
		trainingSet = makeTrainingSet(releases.subList(0,releases.size()/2));
		testingSet = makeTestingSet(releases.subList(0,releases.size()/2));
